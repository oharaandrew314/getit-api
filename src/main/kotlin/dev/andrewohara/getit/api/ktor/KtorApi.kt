...
fun Application.createRoutes(service: GetItService) = routing {
    authenticate("Bearer") {
        ...
        post<ShoppingListsResource> {
            authorize()
                .flatMap { service.createList(it, call.receive<ShoppingListDataDtoV1>().toModel()) }
        ...
        put<ShoppingListsResource.ListId> { id ->
            authorize()
                .flatMap { service.updateList(it, id.toValue(), call.receive<ShoppingListDataDtoV1>().toModel()) }
                .map { it.toDtoV1() }
                .respondTo(call)
        }
        ...
        post<ShoppingListsResource.ListId.Items> { (listId) ->
            authorize()
                .flatMap { service.addItem(it, listId.toValue(), call.receive<ShoppingItemDataDtoV1>().toModel()) }
                .map { it.toDtoV1() }
                .respondTo(call)
        }
        ...
        put<ShoppingListsResource.ListId.Items.ItemId> { itemId ->
            authorize()
                .flatMap { service.updateItem(it, itemId.items.listId.toValue(), itemId.toValue(), call.receive<ShoppingItemDataDtoV1>().toModel()) }
                .map { it.toDtoV1() }
                .respondTo(call)
        }
